...
class CaptioningSolver(object):
    def __init__(self, model, data, **kwargs):
        ...
        if self.update_rule == 'adam':
            ...
            self.optimizer = tf.train.GradientDescentOptimizer     ##### by LY, 2017.09.23
    ...
    def train(self):
        ...
        with tf.name_scope('optimizer'):
            ...
            tf.summary.scalar('batch_loss', loss)     ##### by LY, 2017.09.23

        # add the variables into observation
        for var in tf.trainable_variables():
            # tf.histogram_summary(var.op.name, var)
            tf.summary.histogram(var.op.name, var)     ##### by LY, 2017.09.23
        for grad, var in grads_and_vars:
            # tf.histogram_summary(var.op.name + '/gradient', grad)
            tf.summary.histogram(var.op.name + '/gradient', grad)     ##### by LY, 2017.09.23
    ...
    def all_model_val(self):
        ...
        models = [per for per in list(os.walk(models_path))[0][-1] if per != 'checkpoint' and per[-4::] != 'meta' and per[-1] != '1']     ##### by LY, 2017.09.24
